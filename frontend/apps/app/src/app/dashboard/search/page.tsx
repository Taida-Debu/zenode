...
export default function GitHubSearch() {
  ...
  useEffect(() => {
    fetch('/api/ai/health')
      .then((r) => r.json())
      .then((d) => setAiConfigured(Boolean(d.groq)))
      .catch(() => setAiConfigured(false));
  }, []);

  // Skills-first: search then recommend when skills change
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const data = await aiProjectSearchService.searchProjects(skills);
        if (cancelled) return;
        setResults(data);
        const recs = await aiProjectSearchService.getRecommendations(skills, data);
  ...
  const generateRecommendations = async () => {
    ...
    try {
      const data =
        results?.repositories?.items?.length
          ? results
          : await aiProjectSearchService.searchProjects(skills);
      setResults(data);
      const recs = await aiProjectSearchService.getRecommendations(skills, data);
    ...
    <SidebarProvider>
      ...
      <SidebarInset>
                ...
                <div className="max-w-6xl mx-auto p-4">
                  ...
                  {aiConfigured === false && (
                    <div className="mb-6 glass-effect border border-yellow-500/30 p-4 rounded-xl flex items-start gap-3">
                      <AlertCircle className="w-5 h-5 text-yellow-400 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-white font-medium">AI search is not configured</p>
                        <p className="text-sm text-gray-400 mt-1">
                          Add <code className="text-green-400">GROQ_API_KEY</code> to your server <code className="text-green-400">.env</code> and restart the dev server to enable recommendations and chat.
                        </p>
                      </div>
                    </div>
                  )}
