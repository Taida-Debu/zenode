...
export default function SubmitProposalPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!address) {
      ...
      setSubmitMessage('Connect your wallet before submitting a proposal.')
      return
    }
    ...
    try {
      const res = await fetch('/api/proposals', {
        ...
        body: JSON.stringify({
          ...
          uri: formData.uri,
          contributors: formData.contributors.filter(Boolean),
      ...
      setSubmitStatus('error')
      setSubmitMessage(err instanceof Error ? err.message : 'Failed to submit proposal')
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="pt-4">
                <button
                  ...
                  className="w-full bg-gradient-to-r from-green-400 to-cyan-400 text-white px-6 py-3 rounded-lg font-semibold transition-all duration-300 hover:scale-105 flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  <FileText className="w-5 h-5" />
                  {submitStatus === 'submitting' ? 'Submitting…' : 'Submit Proposal'}
                </button>
              </div>
            </form>
          </div>
